...
var getChannelPreviewDisplayAvatar = function getChannelPreviewDisplayAvatar(channel, client) {
  ...
  if (channelImage) {
    ...
    if (otherMembers.length === 1) {
      var _otherMembers$0$user, _otherMembers$0$user2;

      return {
        image: (_otherMembers$0$user = otherMembers[0].user) == null ? void 0 : _otherMembers$0$user.image,
        name: channelName || ((_otherMembers$0$user2 = otherMembers[0].user) == null ? void 0 : _otherMembers$0$user2.name)
      };
    }
    ...
    return {
      images: otherMembers.slice(0, 4).map(function (member) {
        ...
        return ((_member$user2 = member.user) == null ? void 0 : _member$user2.image) || '';
      }),
      names: otherMembers.slice(0, 4).map(function (member) {
        var _member$user3;

        return ((_member$user3 = member.user) == null ? void 0 : _member$user3.name) || '';
      })
    };
  }
