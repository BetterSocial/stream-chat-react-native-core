...
var useCreateChannelsContext = function useCreateChannelsContext(_ref) {
  ...
  var channelValueString = channels.map(function (channel) {
    var _channel$data;

    return "" + ((_channel$data = channel.data) == null ? void 0 : _channel$data.name) + Object.values(channel.state.members).map(function (member) {
      var _member$user;
